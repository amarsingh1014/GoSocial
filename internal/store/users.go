...
func (s *Password) Set(plaintext string) error {
	// Hash the password
...
func (s *UsersStore) CreateAndInvite(ctx context.Context, user *User, token string, invitationExpiry time.Duration) error {
	...
	return withTx(s.db, ctx, func(tx *sql.Tx) error {
		...
		//send the invite
		err := s.createUserInvitation(ctx, tx, token, invitationExpiry, user.ID)
...
func (s *UsersStore) getUserByToken(ctx context.Context, tx *sql.Tx, token string) (*User, error) {
	query := `
...
func (s *UsersStore) createUserInvitation(ctx context.Context, tx *sql.Tx, token string, invitationExpiry time.Duration, userID int64) error {
	// Create a new user invitation
	query := `INSERT INTO user_invitations (token, expiry, user_id) VALUES ($1, $2, $3)`

	log.Println("Creating user invitation")
	_, err := tx.ExecContext(ctx, query, token, time.Now().Add(invitationExpiry), userID)
